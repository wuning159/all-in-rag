...
检索策略优化关注的不是“底层数据库索引怎么建”，而是 RAG 系统如何组织知识、如何召回上下文、如何减少误召回，以及如何让 LLM 拿到足够可靠的证据。

### 3.1 上下文扩展：Sentence Window Retrieval 的边界

上下文扩展的典型思路是 Sentence Window Retrieval。
...
原始文档
 -> 按句子拆分
 -> 每一句作为一个向量节点 Node
 -> 每个句子节点在 metadata 中保存前后若干句组成的上下文窗口
 -> 用户提问时，先对单一句子节点做相似度检索
 -> 命中最相关句子后，再用 metadata 中的上下文窗口替换单句内容
 -> 把扩展后的上下文交给 LLM 生成答案
```
...
```text
chunk 太小：检索更精确，但上下文不够。
chunk 太大：上下文更完整，但语义混杂，检索不够精确。
```

Sentence Window Retrieval 的价值在于，它把“检索粒度”和“生成上下文粒度”拆开了：

| 阶段 | 使用内容 | 目的 |
| --- | --- | --- |
| 向量索引 | 单个句子 | 提高语义匹配精度 |
| LLM 生成 | 句子前后窗口 | 补足上下文，提升答案完整性 |

但它不是万能方案，主要有两个边界。

第一个边界：如果文档中多处出现语义相似句子，单句向量检索可能命中错误位置。
...
这些句子在不同章节中语义很接近，仅靠单句向量相似度，可能无法判断用户真正问的是哪个章节、哪一页、哪个检查项。

解决方式通常不是只取 Top1，而是组合：

- Top-K 多召回。
- metadata 过滤，例如 `page`、`section`、`chunk_type`、`table_category`。
- BM25 + 向量的混合检索。
- reranker 二次排序。
- 对相邻或重复窗口做去重、合并。
- LLM 生成前保留来源页码和章节，让答案可追溯。

第二个边界：用户问题本身可能包含多个语义意图。

例如：

```text
整理和整顿分别怎么扣分？
新员工如何执行 5S，并且检查表怎么打分？
```

这类问题不是一个单一检索点，而是多个检索意图。如果只把整个问题 embedding 成一个向量，可能只能召回其中一个方面，或者召回一个语义平均后的结果。

更稳的方式是：

```text
复杂问题
 -> query decomposition 拆成多个子问题
 -> 每个子问题分别检索
 -> 多路召回结果合并
 -> reranker 或规则排序
 -> 组装上下文
...
因此，Sentence Window Retrieval 可以解决“小块检索准但上下文不够”的问题，但不能单独解决“多处相似内容”和“复杂多意图问题”。
...
对于当前 `吉野家5S管理手册` Demo，可以先吸收这个思想，而不一定完整实现 LlamaIndex 的 Sentence Window：

```text
检索时 chunk 可以相对小一点。
命中后返回 chunk 所在页或章节附近的扩展上下文。
第 22 页表格类内容单独结构化处理。
不要只依赖 Top1。
复杂问题可以拆成多个检索问题。
```

### 3.2 混合检索：BM25 与向量检索互相兜底

混合检索的核心是把稀疏检索和密集检索结合起来。
...
| 检索方式 | 解决的问题 | 典型优势 | 典型风险 |
| --- | --- | --- | --- |
| BM25 / 稀疏检索 | 关键词必须命中 | 人名、术语、编号、字段名、表格项更稳 | 不理解同义词和近义词 |
| Embedding / 密集检索 | 表达不同但语义相近 | 能处理“西红柿”和“番茄”这类语义接近问题 | 可能把相似问法误认为相同答案 |
| 混合检索 | 单一检索方式不稳定 | BM25 防止向量乱联想，向量弥补关键词漏召回 | 需要融合排序和参数调试 |

密集向量误召回的本质是：向量检索通常是在语义空间里找最近邻。只要系统必须返回 Top-K，即使知识库里没有真正答案，也会返回“最像的”内容。
...
的原因。它们在句式和语义类型上很接近，但实体不一致。

因此，混合检索不是“让向量检索更像关键词检索”，而是让两类检索各自发挥优势：
...
第一类是 RRF（Reciprocal Rank Fusion，倒数排序融合）。

RRF 不直接比较 BM25 分数和向量相似度分数，而是比较每个结果在各自检索结果里的排名。
...
RRF 的优势在于绕开了“不同检索系统原始分数不可比”的问题。
...
这两个分数不是一个量纲，不能直接相加。即使做归一化，也会引入新的调参问题。RRF 直接用排名融合，所以更稳，也更适合作为默认方案。

第二类是加权线性组合。

它会先对不同检索系统的分数做归一化，再按权重合并：

```text
最终分数 = α * 向量分数 + (1 - α) * BM25 分数
```

这种方式更灵活，但需要调参。它适合业务上明确知道某类问题更依赖哪一边的场景：

| 问题类型 | 更适合的权重倾向 |
| --- | --- |
| 产品编号、人名、字段名、制度条款 | BM25 权重大一些 |
| 概念解释、口语化问题、同义表达 | 向量权重大一些 |
| 普通知识问答 | 两者相对均衡 |

对于当前 `吉野家5S管理手册` Demo，建议是：

```text
默认使用 RRF，先求稳。
如果后续发现某类问题明显需要偏向关键词或语义，再做问题分类和动态权重。
```
...
```text
“整理 10 分有哪些检查项？”
 -> 更依赖 BM25、表格字段、metadata。

“为什么要做 5S？”
 -> 更依赖语义检索。

“5S 是什么意思？”
 -> BM25 和向量检索都可能命中，可以用混合检索融合。
```
...
基于目前学习到的数据准备、文本分块、向量检索、Milvus、ES9、上下文扩展和混合检索内容，下周开始设计 `吉野家5S管理手册` 智能问答 Demo 时，可以先按“可解释、可验证、可演示”的思路设计，而不是一开始追求复杂 Agent 或多数据库架构。
...
用户问题
 ...
 -> ES9 BM25 检索
      - 基于 content 做关键词检索
 -> ES9 向量检索
      - 基于 content_vector 做 kNN / ANN 检索
 -> RRF 融合排序
      - 不直接比较 BM25 分数和向量相似度
      - 按两个结果集中的排名做融合
 -> metadata 过滤 / 加权
...
```text
纯向量检索容易误召回。
纯 BM25 不理解同义表达和口语化问题。
混合检索可以让关键词精确性和语义泛化互相兜底。
metadata 可以把检索范围限制在更可靠的页面、章节或表格类型里。
LLM 只负责基于证据生成答案，不负责替检索层兜底。
```
...
| 问题类型 | 示例 | 推荐检索策略 |
| --- | --- | --- |
| 定义类 | “5S 是什么意思？” | BM25 + 向量检索，RRF 融合 |
| 原因 / 解释类 | “为什么要做 5S？” | 向量检索权重更高，召回概念说明段落 |
| 流程类 | “5S 实施步骤有哪些？” | 章节 metadata + BM25 + 向量检索 |
| 职责类 | “餐厅总经理负责什么？” | BM25 命中岗位关键词，向量补充语义召回 |
| 标准类 | “物品有名有家有哪些要求？” | BM25 优先，保留章节上下文 |
| 检查表类 | “整理 10 分有哪些检查项？” | 第 22 页结构化表格 + `chunk_type=table_row` |
| 扣分类 | “清扫部分发现问题怎么扣分？” | 第 22 页表格 metadata + BM25 精确匹配 |
| 无关问题 | “李四是谁？” | 实体一致性检查 + 拒答 |

第 22 页检查表应该作为 Demo 的重点能力之一，因为它最能体现普通文本 RAG 和结构化 RAG 的差异。
...
第 22 页：
  单独用 pdfplumber 解析表格。
  转成结构化 JSON / Markdown 表格。
  按一级分类、分值、检查项生成 table_row 类型 chunk。
```
...
| 字段 | 作用 |
| --- | --- |
| `content` | chunk 原文，用于 BM25、展示和 LLM 上下文 |
| `content_vector` | embedding 向量，用于 kNN 检索 |
| `source` | 文档来源，例如 `吉野家5S管理手册.pdf` |
| `page` | 页码，用于引用和过滤 |
| `section` | 章节标题 |
| `chunk_type` | `normal_text` / `table_row` / `image_caption` |
| `table_category` | 检查表一级分类，例如整理、整顿、清扫 |
| `score_value` | 检查表分值，例如 10 分、30 分 |
| `score_rule` | 扣分规则或检查标准 |

下周开发优先级建议：

```text
第一步：PDF 正文解析
  - 用 PyMuPDF4LLM / PyMuPDFLoader 提取正文和页码

第二步：正文分块
  - 章节优先
  - chunk_size / chunk_overlap 做长度兜底
  - 保留 source、page、section
...
第四步：Embedding
  - 初版使用同一个中文 embedding 模型处理文档 chunk 和用户问题

第五步：ES9 建库与入库
  - content 用于 BM25
  - content_vector 用于 kNN
  - metadata 用于过滤和引用
...
这些可以作为后续增强项。当前最重要的是先把单 PDF、单知识库、可引用、可拒答的 RAG 主链路跑稳。

### 5.9 RAG 框架选型：Python 与 Java
